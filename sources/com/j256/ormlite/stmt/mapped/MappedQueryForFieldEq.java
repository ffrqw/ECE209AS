...
public class MappedQueryForFieldEq<T, ID> extends BaseMappedQuery<T, ID> {
    ...
    protected MappedQueryForFieldEq(TableInfo<T, ID> tableInfo, String statement, FieldType[] argFieldTypes, FieldType[] resultsFieldTypes, String label) {
    ...
    public T execute(DatabaseConnection databaseConnection, ID id, ObjectCache objectCache) throws SQLException {
        ...
        logArgs(args);
        T castResult = result2;
        return result2;
    }

    public static <T, ID> MappedQueryForFieldEq<T, ID> build(DatabaseType databaseType, TableInfo<T, ID> tableInfo, FieldType idFieldType) throws SQLException {
        if (idFieldType == null) {
        ...
        return new MappedQueryForFieldEq(tableInfo, buildStatement(databaseType, tableInfo, idFieldType), new FieldType[]{idFieldType}, tableInfo.getFieldTypes(), "query-for-id");
    }
