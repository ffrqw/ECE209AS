...
public class ObjectMapper extends ObjectCodec implements Versioned, Serializable {
    ...
    protected ObjectMapper(ObjectMapper src) {
        ...
        this._propertyOverrides = src._propertyOverrides.copy();
        this._mixIns = src._mixIns.copy();
        RootNameLookup rootNames = new RootNameLookup();
        this._serializationConfig = new SerializationConfig(src._serializationConfig, this._mixIns, rootNames, this._propertyOverrides);
        this._deserializationConfig = new DeserializationConfig(src._deserializationConfig, this._mixIns, rootNames, this._propertyOverrides);
    ...
    public MutableConfigOverride configOverride(Class<?> type) {
        return this._propertyOverrides.findOrCreateOverride(type);
    }
    ...
    public ObjectMapper enable(Feature... features) {
        ...
        for (int i$ = 0; i$ < len$; i$++) {
            this._jsonFactory.enable(arr$[i$]);
        }
        return this;
    }
    ...
    public ObjectMapper disable(Feature... features) {
        ...
        int len$ = features.length;
        for (int i$ = 0; i$ < len$; i$++) {
            this._jsonFactory.disable(arr$[i$]);
        }
        return this;
    }
    ...
    public <T> T readValue(JsonParser p, TypeReference<?> valueTypeRef) throws IOException, JsonParseException, JsonMappingException {
        return _readValue(getDeserializationConfig(), p, this._typeFactory.constructType((TypeReference) valueTypeRef));
    }
    ...
    public <T extends TreeNode> T readTree(JsonParser p) throws IOException, JsonProcessingException {
        ...
        T n = (JsonNode) _readValue(cfg, p, JSON_NODE_TYPE);
        if (n == null) {
    ...
    public <T> MappingIterator<T> readValues(JsonParser p, Class<T> valueType) throws IOException, JsonProcessingException {
        return readValues(p, this._typeFactory.constructType((Type) valueType));
    }
    ...
    public <T> T treeToValue(TreeNode n, Class<T> valueType) throws JsonProcessingException {
        try {
            ...
            if (n.asToken() == JsonToken.VALUE_EMBEDDED_OBJECT && (n instanceof POJONode)) {
                TreeNode ob = ((POJONode) n).getPojo();
    ...
    protected Object _convert(Object fromValue, JavaType toValueType) throws IllegalArgumentException {
        ...
        try {
            ...
            if (t == JsonToken.VALUE_NULL) {
                ctxt = createDeserializationContext(p, deserConfig);
                nullValue = _findRootDeserializer(ctxt, toValueType).getNullValue(ctxt);
            } else if (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT) {
                nullValue = null;
            } else {
                ctxt = createDeserializationContext(p, deserConfig);
                nullValue = _findRootDeserializer(ctxt, toValueType).deserialize(p, ctxt);
            }
            p.close();
            return nullValue;
        } catch (IOException e) {
    ...
    protected Object _readValue(DeserializationConfig cfg, JsonParser p, JavaType valueType) throws IOException {
        ...
        if (t == JsonToken.VALUE_NULL) {
            ...
            if (cfg.useRootWrapping()) {
                nullValue = _unwrapAndDeserialize(p, ctxt, cfg, valueType, deser);
            } else {
                nullValue = deser.deserialize(p, ctxt);
            }
        }
        p.clearCurrentToken();
        return nullValue;
    ...
    protected Object _readMapAndClose(JsonParser p0, JavaType valueType) throws IOException {
        ...
        try {
            ...
            DeserializationContext ctxt;
            if (t == JsonToken.VALUE_NULL) {
                ctxt = createDeserializationContext(p, getDeserializationConfig());
                nullValue = _findRootDeserializer(ctxt, valueType).getNullValue(ctxt);
            } else if (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT) {
                nullValue = null;
            } else {
