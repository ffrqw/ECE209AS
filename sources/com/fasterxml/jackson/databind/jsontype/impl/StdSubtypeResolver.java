...
public class StdSubtypeResolver extends SubtypeResolver implements Serializable {
    ...
    public Collection<NamedType> collectAndResolveSubtypesByTypeId(MapperConfig<?> config, AnnotatedMember property, JavaType baseType) {
        ...
        _collectAndResolveByTypeId(AnnotatedClass.constructWithoutSuperTypes(rawBase, config), new NamedType(rawBase, null), config, typesHandled, byName);
        Collection<NamedType> st = ai.findSubtypes(property);
        if (st != null) {
            for (NamedType nt : st) {
                _collectAndResolveByTypeId(AnnotatedClass.constructWithoutSuperTypes(nt.getType(), config), nt, config, typesHandled, byName);
            }
        }
        if (this._registeredSubtypes != null) {
            i$ = this._registeredSubtypes.iterator();
