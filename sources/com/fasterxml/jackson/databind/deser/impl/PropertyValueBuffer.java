...
public class PropertyValueBuffer {
    ...
    public final boolean hasParameter(SettableBeanProperty prop) {
        ...
        if (((this._paramsSeen >> prop.getCreatorIndex()) & 1) == 1) {
            return true;
        }
        return false;
    }
    ...
    public Object getParameter(SettableBeanProperty prop) throws JsonMappingException {
        ...
        if (hasParameter(prop)) {
            ...
            value = _findMissing(prop);
            objArr[creatorIndex] = value;
        }
        if (value != null || !this._context.isEnabled(DeserializationFeature.FAIL_ON_NULL_CREATOR_PROPERTIES)) {
            return value;
        }
        throw this._context.mappingException("Null value for creator property '%s'; DeserializationFeature.FAIL_ON_NULL_FOR_CREATOR_PARAMETERS enabled", prop.getName(), Integer.valueOf(prop.getCreatorIndex()));
    }
        ...
        if (this._context.isEnabled(DeserializationFeature.FAIL_ON_NULL_CREATOR_PROPERTIES)) {
            for (ix = 0; ix < props.length; ix++) {
                if (this._creatorParameters[ix] == null) {
                    this._context.reportMappingException("Null value for creator property '%s'; DeserializationFeature.FAIL_ON_NULL_FOR_CREATOR_PARAMETERS enabled", props[ix].getName(), Integer.valueOf(props[ix].getCreatorIndex()));
                }
            }
        }
