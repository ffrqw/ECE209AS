...
final class NativeCrashWriter {
    ...
    public static void writeNativeCrash(SessionEventData crashEventData, LogFileManager logFileManager, Map<String, String> customAttributes, CodedOutputStream cos) throws IOException {
        ...
        for (i = 0; i < protobufMessageArr.length; i++) {
        ...
        RepeatedMessage repeatedMessage = new RepeatedMessage(protobufMessageArr);
        BinaryImageData[] binaryImageDataArr = crashEventData.binaryImages;
        protobufMessageArr = binaryImageDataArr != null ? new BinaryImageMessage[binaryImageDataArr.length] : EMPTY_BINARY_IMAGE_MESSAGES;
        for (i = 0; i < protobufMessageArr.length; i++) {
            protobufMessageArr[i] = new BinaryImageMessage(binaryImageDataArr[i]);
        }
        ExecutionMessage executionMessage = new ExecutionMessage(signalMessage, repeatedMessage, new RepeatedMessage(protobufMessageArr));
        CustomAttributeData[] customAttributeDataArr = crashEventData.customAttributes;
        Map treeMap = new TreeMap(customAttributes);
        ...
        for (i2 = 0; i2 < customAttributeDataArr2.length; i2++) {
            customAttributeDataArr2[i2] = new CustomAttributeData((String) entryArr[i2].getKey(), (String) entryArr[i2].getValue());
        }
        ApplicationMessage applicationMessage = new ApplicationMessage(executionMessage, createCustomAttributesMessage(customAttributeDataArr2));
        DeviceData deviceData = crashEventData.deviceData;
        if (deviceData == null) {
            nullMessage = new NullMessage();
        } else {
            nullMessage = new DeviceMessage(((float) deviceData.batteryCapacity) / 100.0f, deviceData.batteryVelocity, deviceData.proximity, deviceData.orientation, deviceData.totalPhysicalMemory - deviceData.availablePhysicalMemory, deviceData.totalInternalStorage - deviceData.availableInternalStorage);
        }
        ...
        new EventMessage(crashEventData.timestamp, "ndk-crash", applicationMessage, nullMessage, logMessage).write(cos);
    }
}
