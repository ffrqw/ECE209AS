        ...
        if (this.cipherSuites != null) {
            ...
            if (this.cipherSuites == null) {
                ...
                for (int i2 = 0; i2 < this.cipherSuites.length; i2++) {
                    objArr[i2] = CipherSuite.forJavaName(this.cipherSuites[i2]);
                }
                obj2 = Util.immutableList(objArr);
            }
            cipherSuitesString = obj2.toString();
        } else {
            cipherSuitesString = "[all enabled]";
        }
        if (this.tlsVersions != null) {
            if (this.tlsVersions != null) {
                Object[] objArr2 = new TlsVersion[this.tlsVersions.length];
                while (i < this.tlsVersions.length) {
                    objArr2[i] = TlsVersion.forJavaName(this.tlsVersions[i]);
                    i++;
                }
                obj = Util.immutableList(objArr2);
            }
            tlsVersionsString = obj.toString();
        } else {
            tlsVersionsString = "[all enabled]";
        }
