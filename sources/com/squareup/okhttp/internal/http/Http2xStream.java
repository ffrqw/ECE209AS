...
public final class Http2xStream implements HttpStream {
    ...
    public final void writeRequestHeaders(Request request) throws IOException {
        if (this.stream == null) {
            ...
            if (this.framedConnection.getProtocol() == Protocol.HTTP_2) {
                ...
                requestHeaders = new ArrayList(headers.size() + 4);
                requestHeaders.add(new Header(Header.TARGET_METHOD, request.method()));
                requestHeaders.add(new Header(Header.TARGET_PATH, RequestLine.requestPath(request.httpUrl())));
                requestHeaders.add(new Header(Header.TARGET_AUTHORITY, Util.hostHeader(request.httpUrl())));
                requestHeaders.add(new Header(Header.TARGET_SCHEME, request.httpUrl().scheme()));
    ...
    private static List<Header> spdy3HeadersList(Request request) {
        Headers headers = request.headers();
                ...
                if (names.add(name)) {
                    ...
                    for (int j = 0; j < result.size(); j++) {
                        if (((Header) result.get(j)).name.equals(name)) {
                            result.set(j, new Header(name, new StringBuilder(((Header) result.get(j)).value.utf8()).append('\u0000').append(value).toString()));
                            break;
                        }
                    }
                }
    ...
    private static Builder readSpdy3HeadersList(List<Header> headerBlock) throws IOException {
        String status = null;
