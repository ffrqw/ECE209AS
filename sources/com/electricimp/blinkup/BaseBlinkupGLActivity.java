...
public class BaseBlinkupGLActivity extends Activity {
    ...
    private static class BlinkupHandler extends Handler {
        ...
        public BlinkupHandler(BaseBlinkupGLActivity activity, BlinkupPacket packet) {
    ...
    private class TokenAcquireCallback implements com.electricimp.blinkup.BlinkupController.TokenAcquireCallback {
        private TokenAcquireCallback() {
        }
        ...
        public final void onError(String errorMsg) {
            Toast.makeText(BaseBlinkupGLActivity.this, errorMsg, 1).show();
            Log.e("BlinkUp", "Error getting setup token: " + errorMsg);
        }
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        container.addView(this.countdownPanel);
        ...
        this.countdownPanel.addView(imageView, layoutParams2);
        BlinkupController blinkup = BlinkupController.getInstance();
        if (blinkup.countdownSeconds > 0) {
            this.countdownCounter = blinkup.countdownSeconds;
        }
        ...
        if (blinkup.setFullScreenBrightness) {
            WindowManager.LayoutParams params = getWindow().getAttributes();
            params.screenBrightness = 1.0f;
            getWindow().setAttributes(params);
        }
        this.surfaceView = new BlinkupSurfaceView(this, 640.0f * getResources().getDisplayMetrics().density);
        container.addView(this.surfaceView, 0, new FrameLayout.LayoutParams(-1, -1, 17));
    }

    protected final void startBlinkup() {
        this.blinkupStartTime = System.currentTimeMillis();
        String apiKey = getIntent().getExtras().getString("apiKey");
        if (apiKey != null) {
            this.tokenAcquireCallback = new TokenAcquireCallback();
            BlinkupController.getInstance().acquireSetupToken(this, apiKey, this.tokenAcquireCallback);
            return;
        }
    ...
    private void initBlinkUp(Intent intent) {
        ...
        BlinkupController blinkup = BlinkupController.getInstance();
        blinkup.saveLastMode(intent.getExtras().getString("mode"));
        if (packet == null) {
            finish();
        }
        long delay = 1000 - (System.currentTimeMillis() - this.blinkupStartTime);
        if (delay < 0) {
            delay = 0;
        }
        this.handler = new BlinkupHandler(this, packet);
    ...
    protected final void updateCountdown() {
