...
public class zzamj {
    ...
    private zzamj(zzaml zzaml) {
        ...
        zzaoc.zzbq(new StringBuilder(String.valueOf(str).length() + 134).append("Google Analytics ").append(str).append(" is starting up. To enable debug logging on a device run:\n  adb shell setprop log.tag.GAv4 DEBUG\n  adb logcat -s GAv4").toString());
        ...
        if (zzafM == null) {
            synchronized (zzamj.class) {
                if (zzafM == null) {
                    ...
                    elapsedRealtime = zzrY.elapsedRealtime() - elapsedRealtime;
                    long longValue = ((Long) zzans.zzahU.get()).longValue();
                    if (elapsedRealtime > longValue) {
                        zzamj.zzkr().zzc("Slow initialization (ms)", Long.valueOf(elapsedRealtime), Long.valueOf(longValue));
                    }
                }
            }
        }
    ...
    public final zzaog zzkH() {
        return !this.zzafU.isInitialized() ? null : this.zzafU;
    }
