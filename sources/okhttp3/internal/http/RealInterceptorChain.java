...
public final class RealInterceptorChain implements Chain {
    ...
    public final Response proceed(Request request, StreamAllocation streamAllocation, HttpCodec httpCodec, Connection connection) throws IOException {
        ...
        if (this.httpCodec != null) {
            ...
            if (url.host().equals(this.connection.route().address().url().host()) && url.port() == this.connection.route().address().url().port()) {
                obj = 1;
            } else {
                obj = null;
            }
