...
public class AsyncTimeout extends Timeout {
    ...
    private static final class Watchdog extends Thread {
        ...
        L_0x0016:
            r1 = 0;
            okio.AsyncTimeout.head = null;	 Catch:{ all -> 0x000b }
            monitor-exit(r2);	 Catch:{ all -> 0x000b }
            return;
        L_0x001c:
            monitor-exit(r2);	 Catch:{ all -> 0x000b }
    ...
    private static synchronized void scheduleTimeout(AsyncTimeout node, long timeoutNanos, boolean hasDeadline) {
        synchronized (AsyncTimeout.class) {
            ...
            long remainingNanos = node.timeoutAt - now;
            AsyncTimeout prev = head;
            while (prev.next != null && remainingNanos >= prev.next.timeoutAt - now) {
                prev = prev.next;
            }
            node.next = prev.next;
            prev.next = node;
    ...
    static AsyncTimeout awaitTimeout() throws InterruptedException {
        ...
        if (node == null) {
            ...
            return (head.next != null || System.nanoTime() - startNanos < IDLE_TIMEOUT_NANOS) ? null : head;
        } else {
