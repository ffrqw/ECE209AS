...
final class GapWorker implements Runnable {
    ...
    static class LayoutPrefetchRegistryImpl implements LayoutPrefetchRegistry {
        ...
        final void setPrefetchVector(int dx, int dy) {
            this.mPrefetchDx = dx;
            this.mPrefetchDy = dy;
        }

        final void collectPrefetchPositionsFromView(RecyclerView view, boolean nested) {
            this.mCount = 0;
        ...
        public final void addPosition(int layoutPosition, int pixelDistance) {
            if (layoutPosition < 0) {
                throw new IllegalArgumentException("Layout positions must be non-negative");
            } else if (pixelDistance < 0) {
                throw new IllegalArgumentException("Pixel distance must be non-negative");
            } else {
                int storagePosition = this.mCount << 1;
                if (this.mPrefetchArray == null) {
                    this.mPrefetchArray = new int[4];
                    Arrays.fill(this.mPrefetchArray, -1);
                } else if (storagePosition >= this.mPrefetchArray.length) {
                    int[] oldArray = this.mPrefetchArray;
                    this.mPrefetchArray = new int[(storagePosition << 1)];
        ...
        final boolean lastPrefetchIncludedPosition(int position) {
            if (this.mPrefetchArray != null) {
                int count = this.mCount << 1;
    ...
    final void postFromTraversal(RecyclerView recyclerView, int prefetchDx, int prefetchDy) {
    ...
    private void buildTaskList() {
        ...
        for (i = 0; i < viewCount; i++) {
            ...
            if (view.getWindowVisibility() == 0) {
                ...
                for (int j = 0; j < (prefetchRegistry.mCount << 1); j += 2) {
                    Task task;
                    boolean z;
        ...
        if (innerView != null) {
            ...
            if (innerPrefetchRegistry.mCount != 0) {
                try {
                    ...
                    for (int i = 0; i < (innerPrefetchRegistry.mCount << 1); i += 2) {
    ...
    final void prefetch(long deadlineNs) {
        buildTaskList();
