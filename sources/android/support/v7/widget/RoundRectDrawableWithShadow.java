...
final class RoundRectDrawableWithShadow extends Drawable {
    ...
    final void setShadowSize(float shadowSize, float maxShadowSize) {
        if (shadowSize < 0.0f) {
    ...
    final void setCornerRadius(float radius) {
        if (radius < 0.0f) {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        if (this.mCardBounds.width() - (inset * 2.0f) > 0.0f) {
    ...
    final void getMaxShadowAndCornerPadding(Rect into) {
        getPadding(into);
    }

    final void setShadowSize(float size) {
        setShadowSize(size, this.mRawMaxShadowSize);
    }

    final void setMaxShadowSize(float size) {
        setShadowSize(this.mRawShadowSize, size);
    }

    final float getShadowSize() {
        return this.mRawShadowSize;
    }

    final float getMaxShadowSize() {
    ...
    final void setColor(ColorStateList color) {
        setBackground(color);
        invalidateSelf();
    }

    final ColorStateList getColor() {
