...
final class AdapterHelper implements Callback {
    ...
    final AdapterHelper addUpdateOp(UpdateOp... ops) {
    ...
    final void dispatchFirstPassAndUpdateViewHolders(UpdateOp op, int offsetStart) {
    ...
    final boolean hasAnyUpdateTypes(int updateTypes) {
        return (this.mExistingUpdateTypes & updateTypes) != 0;
    }

    final int findPositionOffset(int position) {
        return findPositionOffset(position, 0);
    }

    final int findPositionOffset(int position, int firstPostponedItem) {
        int count = this.mPostponedList.size();
    ...
    final boolean onItemRangeChanged(int positionStart, int itemCount, Object payload) {
        if (itemCount <= 0) {
            return false;
        }
    ...
    final boolean onItemRangeInserted(int positionStart, int itemCount) {
        if (itemCount <= 0) {
            return false;
        }
    ...
    final boolean onItemRangeRemoved(int positionStart, int itemCount) {
        if (itemCount <= 0) {
            return false;
        }
    ...
    final boolean onItemRangeMoved(int from, int to, int itemCount) {
        if (from == to) {
            return false;
        }
        if (itemCount != 1) {
    ...
    public final int applyPendingUpdatesToPosition(int position) {
    ...
    final void recycleUpdateOpsAndClearList(List<UpdateOp> ops) {
        int count = ops.size();
        for (int i = 0; i < count; i++) {
